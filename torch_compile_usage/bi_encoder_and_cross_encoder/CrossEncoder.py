...
class CrossEncoder(PushToHubMixin):
    ...
    def smart_batching_collate_text_only(self, batch: list[InputExample]) -> BatchEncoding:
        ...
        # tokenized = self.tokenizer(
        #     *texts, padding='max_length', truncation='only_first', return_tensors="pt", max_length=self.max_length
        # )
        tokenized = self.tokenizer(
            *texts, padding='max_length', truncation=True, return_tensors="pt", max_length=self.max_length
        )
