...
class Attention(nn.Module):
    ...
    def forward(
        ...
        k_cache, v_cache = kv_cache[int(layer_id)]
        output = flash_attn_with_kvcache(
            xq, k_cache, v_cache,
            k=xk, v=xv,
            cache_seqlens=curr_idx,
            causal=True,
        )
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        """
        h = x + self.attention(self.attention_norm(x), freqs_cis=freqs_cis, layer_id=layer_id, kv_cache=kv_cache, curr_idx=curr_idx)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        for layer_id, layer in self.layers.items():
            h = layer(h, freqs_cis=self.freqs_cis, layer_id=layer_id, kv_cache=kv_cache, curr_idx=curr_idx)
