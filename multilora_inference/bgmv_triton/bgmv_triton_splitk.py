...
def bgmv_shrink_kernel(
    ...
    pid_k = tl.program_id(axis=2)  # split-k id
    ...
    # Interleaved split-K: each program handles tiles starting at pid_k * BLOCK_K, stepping by BLOCK_K * SPLIT_K
    for k_off in tl.range(0, F_IN, BLOCK_K * SPLIT_K, num_stages=2):
        k = k_off + (pid_k * BLOCK_K) + k0
        k_mask = k < F_IN
        k64 = k.to(tl.int64)
    ...
    # When using split-K, accumulate partials atomically into Y; otherwise follow ADD_TO_Y semantics
    if SPLIT_K == 1:
        if ADD_TO_Y:
        ...
        if ADD_TO_Y:
            ...
            if pid_k == 0:
                ...
                delta = acc - y_old
                if OUT_IS_FP16:
                    delta = delta.to(tl.float16)
                elif OUT_IS_BF16:
                    delta = delta.to(tl.bfloat16)
                else:
                    delta = delta.to(tl.float32)
                tl.atomic_add(y_ptr, delta, mask=b_in & j_in)
            else:
                out = acc
                if OUT_IS_FP16:
                    out = out.to(tl.float16)
...
def bgmv_expand_kernel(
    ...
    # Interleaved split-K across BLOCK_K tiles
    for k_off in tl.range(0, F_IN, BLOCK_K * SPLIT_K, num_stages=2):
        k = k_off + (pid_k * BLOCK_K) + k0
        k_mask = k < F_IN
        k64 = k.to(tl.int64)
    ...
    if SPLIT_K == 1:
        ...
        if ADD_TO_Y:
            ...
            if OUT_IS_FP16:
                ...
                out = out.to(tl.float32)
            tl.atomic_add(y_ptr, out, mask=m_mask)
        else:
            if pid_k == 0:
                y_old = tl.load(y_ptr, mask=m_mask, other=0).to(tl.float32)
                delta = acc - y_old
                if OUT_IS_FP16:
                    ...
                    delta = delta.to(tl.float32)
                tl.atomic_add(y_ptr, delta, mask=m_mask)
            else:
                out = acc
                if OUT_IS_FP16:
                    out = out.to(tl.float16)
...
def bgmv_triton(
    ...
    *,
    ...
    # SPLIT_K is autotuned via meta; runtime does not control it.

    if F_in < F_out:
        # EXPAND
        def grid(meta):
            return (_cdiv(F_OUT_i, meta['BLOCK_M']), B_i, meta['SPLIT_K'])
        ...
        # SHRINK
        def grid(meta):
            return (F_OUT_i, B_i, meta['SPLIT_K'])
