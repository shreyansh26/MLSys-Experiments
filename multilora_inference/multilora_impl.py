...
def run_multilora_test(type: str, dtype: torch.dtype = torch.float16, bench: bool = True):
    ...
    for bs in range(2, 64, 2):
        x = torch.randn(bs, 1, h1, dtype=dtype, device="cuda")
        y = torch.randn(bs, 1, h2, dtype=dtype, device="cuda")
        ...
        if not bench:
            ...
            assert torch.allclose(y_loop_out, y_bgmv_cuda_out, atol=1e-3, rtol=1e-3)
            assert torch.allclose(y_loop_out, y_bgmv_triton_out, atol=1e-3, rtol=1e-3)
