...
class MultiviewKMeans(BaseCluster):
    ...
    *Co-EM Algorithm*
    ...
    def _init_centroids(self, Xs):
        ...
        Parameters
        ----------
        Xs : list of array-likes or cupy.ndarray
            - Xs length: n_views
        ...
        if self.init == 'random':
            ...
            indices = cp.random.choice(Xs[0].shape[0], self.n_clusters)
            ...
            indices.append(cp.random.randint(Xs[1].shape[0]))
            centers2.append(Xs[1][indices[0], :])

            # Compute the remaining n_cluster centroids
            for _ in range(self.n_clusters - 1):
                dists = self._compute_dist(Xs[1], cp.vstack(centers2))
                min_dists = cp.min(dists, axis=1)
                probabilities = min_dists / cp.sum(min_dists)
                cumulative_probabilities = cp.cumsum(probabilities)
                r = cp.random.rand()
                index = cp.searchsorted(cumulative_probabilities, r)
                indices.append(index)
                centers2.append(Xs[1][index])

            centers1 = Xs[0][indices]
            centers2 = cp.vstack(centers2)
            centroids = [centers1, centers2]
        else:
            centroids = self.init
            try:
                centroids = [cp.asarray(view) for view in centroids]
    ...
    def _final_centroids(self, Xs, centroids):
        ...
        for clust in range(self.n_clusters):
            ...
            v1_partitions = cp.argmin(v1_distances, axis=1).flatten()
            v2_distances = self._compute_dist(Xs[1], centroids[1])
            v2_partitions = cp.argmin(v2_distances, axis=1).flatten()
            ...
            if cp.sum(part_indices) != 0:
                cent1 = cp.mean(Xs[0][part_indices], axis=0)
                v1_consensus.append(cent1)

                cent2 = cp.mean(Xs[1][part_indices], axis=0)
        ...
        if len(v1_consensus) == 0:
            ...
            self.centroids_[0] = cp.vstack(v1_consensus)
            self.centroids_[1] = cp.vstack(v2_consensus)
    ...
    def _em_step(self, X, partition, centroids):
        ...
        for cl in range(self.n_clusters):
            ...
            if cp.sum(mask) == 0:
                ...
                cent = cp.mean(X[mask], axis=0)
                new_centers.append(cent)
        new_centers = cp.vstack(new_centers)

        # Compute expectation and objective function
        distances = self._compute_dist(X, new_centers)
        new_parts = cp.argmin(distances, axis=1).flatten()
        min_dists = distances[cp.arange(n_samples), new_parts]
        o_funct = cp.sum(min_dists)
    ...
    def _preprocess_data(self, Xs):
        ...
        # Check that the input data is valid and convert to cupy arrays
        Xs_new = check_Xs(Xs, enforce_views=2)
        Xs_new = [cp.asarray(view) for view in Xs_new]
        return Xs_new
    ...
    def _one_init(self, Xs):
        ...
        -------
        inertia: float
            The final inertia for this run.
        ...
        parts = cp.argmin(distances, axis=1).flatten()
        partitions = [None, parts]
        objective = cp.array([cp.inf, cp.inf])
        o_funct = [None, None]
        iter_stall = [0, 0]
        iter_num = 0
        max_iter = cp.inf
        ...
        while max(iter_stall) < self.patience and iter_num < max_iter:
            ...
            for view in range(2):
                if (objective[view] - o_funct[view] > self.tol * cp.abs(
                        objective[view])):
    ...
    def fit(self, Xs, y=None):
        ...
        run_results = Parallel(n_jobs=self.n_jobs)(delayed(self._one_init)(Xs) for _ in range(n_init))

        # Zip results and find which has max inertia
        inertias, centroids = zip(*run_results)
        min_index = cp.argmin(cp.array(inertias)).item()

        # Compute final cluster centroids
        self._final_centroids(Xs, centroids[min_index])
    ...
    def predict(self, Xs):
        ...
        labels = cp.argmin(dist_metric, axis=1).flatten()
