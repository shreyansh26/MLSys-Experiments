...
def check_Xs(
    ...
    r"""
    Checks Xs and ensures it is a list of 2D matrices.

    Parameters
    ----------
    Xs : list of arrays
        Input data. Each element of the list should be a NumPy array or a CuPy array.
    ...
    Returns
    -------
    Xs_converted : list of arrays
        The converted and validated Xs.
    ...
    if not isinstance(Xs, list):
        if hasattr(Xs, 'ndim'):
            ...
            msg = f"If not list, input must be array-like, not {type(Xs)}"
            raise ValueError(msg)
    ...
    for i, X in enumerate(Xs):
        if not hasattr(X, 'ndim'):
            msg = f"Xs[{i}] must be an array with ndim attribute."
            raise ValueError(msg)
        if X.ndim != 2:
            msg = f"Xs[{i}] must be a 2D array, but got array with shape {X.shape}"
            raise ValueError(msg)
        if copy:
            Xs[i] = X.copy()

    # Check that all views have the same number of samples
    n_samples = Xs[0].shape[0]
    for X in Xs:
