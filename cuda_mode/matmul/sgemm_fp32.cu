...
int main(int argc, char **argv) {
    ...
    for(int size : SIZE) {
        ...
        if(kernel_num != 0) {
            run_kernel_fp32(0, m, n, k, alpha, A_d, B_d, beta, C_ref_d, handle); // cuBLAS
            run_kernel_fp32(kernel_num, m, n, k, alpha, A_d, B_d, beta, C_d, handle); // Executes the kernel, modifies the result matrix
        ...
        for (int j = 0; j < repeat_times; j++) {
            ...
            run_kernel_fp32(kernel_num, m, n, k, alpha, A_d, B_d, beta, C_d, handle);
        }
        cudaEventRecord(end);
