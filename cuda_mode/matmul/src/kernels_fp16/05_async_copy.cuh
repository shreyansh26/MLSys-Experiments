...
__global__ void async_copy(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M, const unsigned int N, unsigned int K) {
    ...
    for(unsigned int block_k = 1; block_k <= num_block_tiles_k; block_k++) {
        if(block_k != num_block_tiles_k) {
            ...
            tileMemcpyLoad<BM_dim, BK_dim, NUM_THREADS, 8>(A_block_gmem, A_gmem_temp_reg, K);
        ...
        if(block_k != num_block_tiles_k) {
            tileMemcpySwizzledStore<BM_dim, BK_dim, NUM_THREADS, SWIZZLE_BITS_A, 8>(A_gmem_temp_reg, A_block_smem);
