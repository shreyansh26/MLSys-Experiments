...
__global__ void __launch_bounds__(NUM_THREADS) async_stores(int M, int N, int K, bf16* C, const CUtensorMap* tensorMapC, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, float alpha, float beta, int *DB) {
    ...
    SMemQueue2<BM, BN, BK, QSIZE> &s = *reinterpret_cast<SMemQueue2<BM, BN, BK, QSIZE>*>(smem_queue);
    bf16 *sA = s.A;
    bf16 *sB = s.B;
    bf16 *sC = s.C;
        ...
        // Store
        {
            ...
            for (int m_it = 0; m_it < B_WG_M/WGMMA_M; ++m_it) {
                ...
                for (int w = 0; w < WGMMA_N/16; ++w) {
                    ...
                    #define IDX(i, j) ((j)*B_WG_M + ((i) + offset_m))
                    #define ST(i, j, v) block_sC[IDX(i, j)] = v;

                    ST(row, col, d[m_it][w][0]);
                    ST(row+8, col, d[m_it][w][2]);
                    ST(row, col+1, d[m_it][w][1]);
                    ST(row+8, col+1, d[m_it][w][3]);

                    ST(row, col+8, d[m_it][w][4]);
                    ST(row+8, col+8, d[m_it][w][6]);
                    ST(row, col+9, d[m_it][w][5]);
                    ST(row+8, col+9, d[m_it][w][7]);
