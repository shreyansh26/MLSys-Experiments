...
__global__ void __launch_bounds__(NUM_THREADS) tensor_core_matmul(int M, int N, int K, bf16* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB, float alpha, float beta) {
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k; ++block_k_iter) {
        ...
        // Compute
        warpgroup_arrive();
        wgmma64<1, 1, 1, 0, 1>(d, &sA[0], &sB[0]);
        wgmma64<1, 1, 1, 0, 1>(d, &sA[WGMMA_K], &sB[WGMMA_K]);
        wgmma64<1, 1, 1, 0, 1>(d, &sA[2*WGMMA_K], &sB[2*WGMMA_K]);
        wgmma64<1, 1, 1, 0, 1>(d, &sA[3*WGMMA_K], &sB[3*WGMMA_K]);
