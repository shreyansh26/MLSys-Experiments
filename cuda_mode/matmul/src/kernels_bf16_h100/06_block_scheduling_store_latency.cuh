...
__global__ __launch_bounds__(NUM_THREADS) void block_scheduling_store_latency(int M, int N, int K, bf16* C, const __grid_constant__ CUtensorMap tensorMapA, const __grid_constant__ CUtensorMap tensorMapB, float alpha, float beta, int *DB) {
    ...
    if(wg_idx == 0) { // producer
        ...
        */
        ...
        if(tid == 0) {
            int qidx = 0;
            for(int num_block = schedule.next(); num_block >= 0; num_block = schedule.next()) { // returns -1 when done
