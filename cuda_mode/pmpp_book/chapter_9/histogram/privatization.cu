...
__global__ void histo_kernel(int *data, unsigned int N, unsigned int *histo) {
    ...
    if(i < N) {
        ...
        atomicAdd(&(histo[blockIdx.x * BIN_COUNT + idx]), 1);
    }

    if(blockIdx.x > 0) {
        __syncthreads();
        for(int bin=threadIdx.x; bin<BIN_COUNT; bin+=blockDim.x) {
            unsigned int binVal = histo[blockIdx.x * BIN_COUNT + bin];
            if(binVal > 0) {
...
void compute_histogram(int *data_h, unsigned int N, unsigned int *histo_h) {
    ...
    cudaMalloc((void**)&histo_d, cdiv(N, BLOCK_SIZE)*BIN_COUNT*sizeof(unsigned int));

    cudaMemcpy(data_d, data_h, N*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(histo_d, histo_h, cdiv(N, BLOCK_SIZE)*BIN_COUNT*sizeof(unsigned int), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(histo_h, histo_d, cdiv(N, BLOCK_SIZE)*BIN_COUNT*sizeof(unsigned int), cudaMemcpyDeviceToHost);
...
int main() {
    ...
    int *data = new int[N];
    unsigned int *histo = new unsigned int[cdiv(N, BLOCK_SIZE) * BIN_COUNT];
    ...
    for(unsigned int i=0; i<(cdiv(N, BLOCK_SIZE) * BIN_COUNT); i++) {
        histo[i] = 0;
    }
