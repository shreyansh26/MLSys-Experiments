...
void matmul_kernel(float* O, float* A, float* B, int M, int K, int N) {
    int c = blockIdx.x * TILE_WIDTH * COARSE_FACTOR + threadIdx.x;
    ...
    for(int i=0; i<ceil(K/(float)TILE_WIDTH); i++) {
        ...
        for(int j=0; j<COARSE_FACTOR; j++) {
            int c_new = c + j * TILE_WIDTH;
            if((i*TILE_WIDTH + ty) < K and c_new < N)
                Bs[ty][tx] = B[(i*TILE_WIDTH + ty)*N + c_new];
            else
                Bs[ty][tx] = 0.0f;
...
torch::Tensor matmul(torch::Tensor m, torch::Tensor n) {
    ...
    dim3 blocks(cdiv(w, tpb.x*COARSE_FACTOR), cdiv(h, tpb.y));
...
m1 = torch.randn(540, 1098)
m2 = torch.randn(1098, 259)

m1c, m2c = m1.contiguous().cuda(), m2.contiguous().cuda()

tr = torch.matmul(m1, m2)
