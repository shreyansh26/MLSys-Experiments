...
torch::Tensor attention_forward_flash(
    torch::Tensor Q,
    torch::Tensor K,
    torch::Tensor V,
    bool is_causal
    ...
    if (Q.dtype() == torch::kFloat32) {
        ...
        launch_flash_attention_kernel<__half>(
        ...
        launch_flash_attention_kernel<__nv_bfloat16>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("attention_forward_flash", &attention_forward_flash, "Flash Attention forward (CUDA)",
