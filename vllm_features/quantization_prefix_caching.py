...
if args.fp8:
    ...
    print("Using fp8 static quantized model!")
else:
    MODEL_NAME = "meta-llama/Meta-Llama-3-8B-Instruct"
    print("Using bf16 model!")
...
questions = [
    ...
    "What was the issue faced by the customer in the conversation?",
...
if args.prefix_caching:
    model = LLM(model=MODEL_NAME, gpu_memory_utilization=0.6, enable_prefix_caching=True)
    print("Prefx caching enabled!")
else:
    model = LLM(model=MODEL_NAME, gpu_memory_utilization=0.6)
    print("No prefix caching!")

sampling_params = SamplingParams(temperature=0, max_tokens=300)

start_time = time.time()
llm_outputs = model.generate(TOTAL_SAMPLES, sampling_params)
end_time = time.time()
...
# No Quantization + No Prefix Caching
# Time per request: 0.045s
# Number of requests in one second: 22.466

# FP8
# Time per request: 0.031s
# Number of requests in one second: 32.120

# Prefix Caching
# Time per request: 0.020s
# Number of requests in one second: 50.796

# FP8 + Prefix Caching
# Time per request: 0.018s
# Number of requests in one second: 55.779